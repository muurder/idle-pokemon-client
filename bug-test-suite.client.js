...
(function () {
    ...
    (function () {   // docas: bloco isolado, fora do bugSuite()
        ...
        if (!window.__docaInventarioInstalada) {
            ...
            const _fimCriar = (typeof docaCronometro === 'function') ? docaCronometro('doca:criada', { doca: 'inventario' }) : (() => {});
            try { docaInventario(); } catch (e) { }
            if (typeof _fimCriar === 'function') _fimCriar();
            docaInvEnvelopar();
            docaInvObservarModal();
            setInterval(docaInvTick, 400);
            if (typeof docaMedir === 'function') docaMedir('doca:instalada', { doca: 'inventario' });
        }
